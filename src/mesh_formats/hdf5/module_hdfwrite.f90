...
INTEGER(HID_T) :: dset_1, dset_2, dset_3, dset_4       
!> Dataset identifier for different datasets that is created
INTEGER(HID_T) :: dspace_1, dspace_2, dspace_3, dspace_4      
!> Dataspace identifier for the dataspace created


INTEGER(HSIZE_T), DIMENSION(2) :: dims_1 
!> Dataset dimensions for the size of array that needs to be generated
INTEGER(HSIZE_T), DIMENSION(2) :: dims_2 
!> Dataset dimensions
INTEGER(HSIZE_T), DIMENSION(1) :: dims_3 
!> Dataset dimensions
INTEGER(HSIZE_T), DIMENSION(1) :: dims_4 
!> Dataset dimensions
INTEGER(ik) ::   rank_1, rank_2, rank_3, rank_4  
!> Dataset rank that tells the dimension of the array

INTEGER(ik) ::   error 
!> Error flag to initilize and to check if any error is generated while compiling file

INTEGER(ik) :: i, n

REAL(rk), allocatable :: cord(:,:), vel(:,:), pre(:) 
...
!open(1, file="puntosmalla.dat", status='old', action='read') 
...
!> creating data set for node numbers that needs to be saved that is required to save the data



dims_1(1:2) = (/3, n/) 
!> assigning the size of the data set that needs to be written
CALL h5dwrite_f(dset_1, H5T_NATIVE_REAL, cord, dims_1, error)
!> writing the coordinate data to the file 
...
!> write node number data to the file 


CALL h5dclose_f(dset_1, error) 
!> closing the data space which limits the modification of size of the data 
CALL h5dclose_f(dset_2, error)  
CALL h5dclose_f(dset_3, error)  
CALL h5dclose_f(dset_4, error)  

CALL h5sclose_f(dspace_1, error)  
!> closing the dataset which limits the writing permission  
