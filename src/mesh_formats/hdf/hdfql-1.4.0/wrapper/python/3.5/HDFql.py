...
def variable_register(numpy_array):
	...
	if numpy_array.dtype.type not in [_numpy.int8, _numpy.uint8, _numpy.int16, _numpy.uint16, _numpy.int32, _numpy.uint32, _numpy.int64, _numpy.uint64, _numpy.float32, _numpy.float64, _numpy.string_]:
	...
	_HDFql.variable_register(slot, address)
	_variable_list[slot] = address
	return slot
...
def variable_get_datatype(numpy_array):
	try:
		address = numpy_array.__array_interface__["data"][0]
	except:
		return _HDFql.ERROR_NO_ADDRESS
	for i in range(16):
		if address == _variable_list[i]:
			return _HDFql.variable_get_datatype(i)
	return _HDFql.ERROR_NOT_REGISTERED
...
def execute(script):
	try:
		return _HDFql.execute(script)
	except:
		try:
			return _HDFql.execute(script.encode("utf-8"))
		except:
			return _HDFql.ERROR_UNKNOWN
