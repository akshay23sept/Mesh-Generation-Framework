module fruit
  ...
  !====== begin of generated interface ======
    ...
    module procedure assert_eq_3d_double_in_range2_
...
999 continue
  ...
  !------ 3d_double ------
  subroutine assert_eq_3d_double_in_range2_(var1, var2, n, m, p, delta, message)
    integer, intent (in) :: n, m, p
    integer              :: i, j, k
    double precision, intent (in) :: var1(n, m, p), var2(n, m, p)
    double precision, intent (in) :: delta(n, m, p)
    character(len = *), intent (in), optional :: message
    do k = 1, p
      do j = 1, m
        do i = 1, n
          if (abs(var1(i, j, k) - var2(i, j, k)) > abs(delta(i, j, k))) then
            call failed_assert_action(&
            & to_s(var1(i, j, k)), &
            & to_s(var2(i, j, k)), '3d array has difference, ' // message, if_is = .true.)
            print*, abs(var1(i, j, k) - var2(i, j, k)),  delta(i, j, k)
            return
          endif
        enddo
      enddo
    enddo
    call add_success
  end subroutine assert_eq_3d_double_in_range2_
